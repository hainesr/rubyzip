...
class ZipLocalEntryTest < Minitest::Test
  CEH_FILE = 'test/zip/data/generated/centralEntryHeader.bin'
  LEH_FILE = 'test/zip/data/generated/localEntryHeader.bin'
  ...
  def test_read_date_time
    ::File.open('test/zip/data/rubycode.zip', 'rb') do |file|
      entry = ::Zip::Entry.read_local_entry(file)
  ...
  def test_read_local_entry_from_non_zip_file
    ::File.open('test/zip/data/file2.txt') do |file|
