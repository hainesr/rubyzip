...
class FileNonmutatingTest < Minitest::Test
  def setup
    @zipsha = Digest::SHA1.file('test/zip/data/zipWithDirs.zip')
    @zip_file = ::Zip::File.new('test/zip/data/zipWithDirs.zip')
  end

  def teardown
    @zip_file.close if @zip_file
    assert_equal(@zipsha, Digest::SHA1.file('test/zip/data/zipWithDirs.zip'))
  end
  ...
  def test_symlink?
    zip_file = ::Zip::File.new('test/zip/data/path_traversal/tuzovakaoff/symlink.zip')
  ...
  def test_zero?
    ...
    block_called = false
    ::Zip::File.open('test/zip/data/generated/5entry.zip') do |zf|
      block_called = true
      assert(zf.file.zero?('test/zip/data/generated/empty.txt'))
    end
    assert(block_called)
    ...
    ::Zip::File.open('test/zip/data/generated/5entry.zip') do |zf|
      block_called = true
      assert(zf.file.stat('test/zip/data/generated/empty.txt').zero?)
    end
    assert(block_called)
  end

  def test_expand_path
    ::Zip::File.open('test/zip/data/zipWithDirs.zip') do |zf|
  ...
  def test_ntfs_time
    ::Zip::File.open('test/zip/data/ntfs.zip') do |zf|
  ...
  def test_foreach
    ::Zip::File.open('test/zip/data/generated/zipWithDir.zip') do |zf|
      ref = []
      File.foreach('test/zip/data/file1.txt') { |e| ref << e }
      index = 0

      zf.file.foreach('test/zip/data/file1.txt') do |l|
  ...
  def test_glob
    ::Zip::File.open('test/zip/data/globTest.zip') do |zf|
      {
  ...
  def test_readlines
    ::Zip::File.open('test/zip/data/generated/zipWithDir.zip') do |zf|
      orig_file = ::File.readlines('test/zip/data/file1.txt')
      zip_file = zf.file.readlines('test/zip/data/file1.txt')
  ...
  def test_read
    ::Zip::File.open('test/zip/data/generated/zipWithDir.zip') do |zf|
      orig_file = ::File.read('test/zip/data/file1.txt')

      # Ruby replaces \n with \r\n automatically on windows
      zip_file = if Zip::RUNNING_ON_WINDOWS
                   zf.file.read('test/zip/data/file1.txt').gsub("\r\n", "\n")
                 else
                   zf.file.read('test/zip/data/file1.txt')
                 end
      assert_equal(orig_file, zip_file)
    end
  end
