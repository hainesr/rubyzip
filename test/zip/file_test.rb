...
class ZipFileTest < Minitest::Test
  ...
  OK_DELETE_FILE = 'test/zip/data/generated/okToDelete.txt'
  OK_DELETE_MOVED_FILE = 'test/zip/data/generated/okToDeleteMoved.txt'
  ...
  def test_get_input_stream_stored_with_gpflag_bit3
    ::Zip::File.open('test/zip/data/gpbit3stored.zip') do |zf|
  ...
  def test_get_output_stream
    ...
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      ...
      ostream =
        zf.get_output_stream(zf.get_entry('test/zip/data/generated/empty.txt'))
      ostream.write 'Putting stuff in data/generated/empty.txt'
      ostream.close

      assert_equal(count + 1, zf.size)
      assert_equal('Putting stuff in data/generated/empty.txt', zf.read('test/zip/data/generated/empty.txt'))
      ...
      zf.get_output_stream('entry.bin') do |os|
        os.write(::File.binread('test/zip/data/generated/5entry.zip'))
      end
    end
    ...
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      ...
      assert_equal('Putting stuff in new_entry.txt', zf.read('new_entry.txt'))
      assert_equal('Putting stuff in data/generated/empty.txt', zf.read('test/zip/data/generated/empty.txt'))
      assert_equal(File.binread('test/zip/data/generated/5entry.zip'), zf.read('entry.bin'))
    end
  end
  ...
  def test_new_with_io_opened_non_binary_mode
    File.open('test/zip/data/test.xlsx') do |io|
  ...
  def test_open_buffer_with_string
    data = File.read('test/zip/data/rubycode.zip', mode: 'rb')
    string = data.dup
  ...
  def test_close_buffer_with_stringio
    string_io = StringIO.new File.read('test/zip/data/rubycode.zip', mode: 'rb')
  ...
  def test_open_buffer_no_op_does_not_change_file
    Dir.mktmpdir do |tmp|
      test_zip = File.join(tmp, 'test.zip')
      FileUtils.cp 'test/zip/data/rubycode.zip', test_zip
      ...
      assert_equal \
        File.binread('test/zip/data/rubycode.zip'),
  ...
  def test_open_buffer_with_io_and_block
    File.open('test/zip/data/rubycode.zip') do |io|
      refute(io.binmode?) # We open in non-binmode on purpose.
  ...
  def test_open_file_with_max_length_comment
    ...
    Zip::File.open('test/zip/data/max_length_file_comment.zip') do |zf|
  ...
  def test_open_zip64_file_with_max_length_comment
    # Should not raise any errors.
    Zip::File.open('test/zip/data/zip64_max_length_file_comment.zip') do |zf|
  ...
  def test_warn_count_entries_with_too_high_number_of_entries_declared
    assert_output('', /Zip consistency problem:/) do
      Zip::File.count_entries('test/zip/data/zipWithTooBigCDirSizeField.zip')
    end

    assert_output('', /Zip consistency problem:/) do
      Zip::File.count_entries('test/zip/data/zipWithTooBigCDirSizeField64.zip')
    end
  end
  ...
  def test_add_different_compression
    src_file = 'test/zip/data/file2.txt'
    entry_name = 'newEntryName.rb'
    files = [
      ['test/zip/data/fast_comp.zip', Zlib::BEST_SPEED],
      ['test/zip/data/default_comp.zip', Zlib::DEFAULT_COMPRESSION],
      ['test/zip/data/best_comp.zip', Zlib::BEST_COMPRESSION]
    ]
    sizes = []
  ...
  def test_recover_permissions_after_add_files_to_archive
    ...
    zf.add('newEntryName.rb', 'test/zip/data/file2.txt')
    zf.close
  ...
  def test_add_existing_entry_name
    error = assert_raises(::Zip::EntryExistsError) do
      ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
        zf.add(zf.entries.first.name, 'test/zip/data/file2.txt')
      end
    end
  ...
  def test_add_existing_entry_name_replace
    ...
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      ...
      zf.add(replaced_entry, 'test/zip/data/file2.txt') do
        called = true
        true
      end
    end
    assert(called)
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      assert_contains(zf, replaced_entry, 'test/zip/data/file2.txt')
    end
  end
  ...
  def test_replace
    ...
    replace_src = 'test/zip/data/file2.txt'
    zf = ::Zip::File.new(TEST_ZIP.zip_name)
  ...
  def test_replace_non_entry
    ...
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      assert_raises(Errno::ENOENT) do
        zf.replace(replace_entry, 'test/zip/data/file2.txt')
      end
    end
  end
  ...
  def test_commit_preserves_options
    zip_file = 'test/zip/data/generated/preserve_options.zip'
  ...
  def test_double_commit(filename = 'test/zip/data/generated/double_commit_test.zip')
    ::FileUtils.touch('test/zip/data/generated/test_double_commit1.txt')
    ::FileUtils.touch('test/zip/data/generated/test_double_commit2.txt')
    zf = ::Zip::File.open(filename, create: true)
    zf.add('test1.txt', 'test/zip/data/generated/test_double_commit1.txt')
    zf.commit
    refute(zf.commit_required?)
    zf.add('test2.txt', 'test/zip/data/generated/test_double_commit2.txt')
    assert(zf.commit_required?)
    zf.commit
    refute(zf.commit_required?)
    zf.close
  ...
  def test_double_commit_zip64
    test_double_commit('test/zip/data/generated/double_commit_test64.zip')
  end
  ...
  def test_streaming
    ...
    zname = 'test/zip/data/generated/README.zip'
  ...
  def test_odd_extra_field
    entry_count = 0
    File.open 'test/zip/data/oddExtraField.zip', 'rb' do |zip_io|
  ...
  def test_open_xlsx_does_not_raise_type_error
    ::Zip::File.open('test/zip/data/test.xlsx')
  end
  ...
  def test_find_get_entry
    ::Zip::File.open(TEST_ZIP.zip_name) do |zf|
      ...
      refute_nil zf.find_entry('test/zip/data/generated/empty.txt')
      ...
      zf.get_entry('test/zip/data/generated/empty.txt')
    end
  end
