...
class ZipEntryTest < Minitest::Test
  ...
  def test_entry_name_cannot_start_with_slash
    ...
    assert_match(%r{'/hej/der'}, error.message)
  end
  ...
  def test_store_file_without_compression
    Dir.mktmpdir do |tmp|
      ...
      zipfile.add(mimetype_entry, 'test/zip/data/mimetype')

      files = Dir[File.join('test/zip/data/globTest', '**', '**')]
      files.each do |file|
        zipfile.add(file.sub('test/zip/data/globTest/', ''), file)
      end
      ...
      assert_match(%r{mimetypeapplication/epub\+zip}, first_100_bytes)
    end
  end
  ...
  def test_ensure_entry_time_set_to_file_mtime
    entry = ::Zip::Entry.new
    entry.gather_fileinfo_from_srcpath('test/zip/data/mimetype')
    assert_equal(entry.time, File.stat('test/zip/data/mimetype').mtime)
  end
