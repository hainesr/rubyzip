...
class DeflaterTest < Minitest::Test
  ...
  DEFLATER_TEST_FILE = 'test/zip/data/generated/deflatertest.bin'
  BEST_COMP_FILE = 'test/zip/data/generated/compressiontest_best_compression.bin'
  DEFAULT_COMP_FILE = 'test/zip/data/generated/compressiontest_default_compression.bin'
  NO_COMP_FILE = 'test/zip/data/generated/compressiontest_no_compression.bin'
  ...
  def test_output_operator
    txt = load_file('test/zip/data/file2.txt')
  ...
  def test_data_error
    assert_raises(::Zip::DecompressionError) do
      inflate('test/zip/data/file1.txt.corrupt.deflatedData')
    end
  end
