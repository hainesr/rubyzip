...
class ZipCentralDirectoryEntryTest < Minitest::Test
  def test_read_from_stream
    File.open('test/zip/data/testDirectory.bin', 'rb') do |file|
  ...
  def test_read_entry_from_truncated_zip_file_raises_error
    File.open('test/zip/data/testDirectory.bin') do |f|
  ...
  def test_read_corrupted_entry_raises_error
    fragment = File.binread('test/zip/data/testDirectory.bin')
    fragment.slice!(12)
